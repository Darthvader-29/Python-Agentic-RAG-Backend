...
class PineconeClient:
    ...
    # ── session helpers (transitional; Phase 2 moves state to Postgres) ──────

    @retry(**_RETRY)
    def _list_s3_keys_sync(self, session_id: str) -> list[str]:
        index = self._index_or_raise()
        try:
            results = index.query(
                vector=[0.0] * 384,
    ...
    async def list_s3_keys_for_session(self, session_id: str) -> list[str]:
        return await asyncio.to_thread(self._list_s3_keys_sync, session_id)

    @retry(**_RETRY)
    def _has_session_documents_sync(self, session_id: str) -> bool:
        index = self._index_or_raise()
        try:
    ...
    async def has_session_documents(self, session_id: str) -> bool:
        return await asyncio.to_thread(self._has_session_documents_sync, session_id)
